...
public class Cono {
    ...
    public float[] coloresAU(int n) {
        ...
        for (int i = 0; i < this.coloresAU.length; i++) {
            coloresAU[i] = aux1[0];
            coloresAU[i + 1] =aux1[1];
            coloresAU[i + 2] =aux1[2];
            coloresAU[i + 3] = aux1[3];
            i += 3;
        }
        return coloresAU;
    }
    ...
    public Cono(int franjas, float radius, float height, Context contexto, float[] matrizProyeccion,
                float[] matrizVista, float[] matrizModelo, float[] colorCono) {
        aux1 = colorCono;
    ...
    public void dibujar(GLES20 gl) {
        ...
        gl.glVertexAttribPointer(idFragmentShader, comPorColores, gl.GL_FLOAT, false, 0, bufferColores);
